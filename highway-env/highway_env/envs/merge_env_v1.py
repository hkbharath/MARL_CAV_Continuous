...
class MergeEnv(AbstractEnv):
    ...
    def _make_road(
        ...
        net.add_lane(
            "a", "b", StraightLane([0, 0], [sum(self.ends[:2]), 0], line_types=[c, c])
        )
        net.add_lane(
            "b",
            "c",
            StraightLane(
                [sum(self.ends[:2]), 0], [sum(self.ends[:3]), 0], line_types=[c, s]
            ),
        )
        net.add_lane(
            "c",
            "d",
            StraightLane(
                [sum(self.ends[:3]), 0], [sum(self.ends), 0], line_types=[c, c]
            ),
        )
        ...
        amplitude = 3.25
        ljk = StraightLane(
            [0, 6.5 + 4], [self.ends[0], 6.5 + 4], line_types=[c, c], forbidden=True
        )
        lkb = SineLane(
    ...
    def _make_vehicles(self, num_CAV=4, num_HDV=3) -> None:
        ...
        spawn_point_m_c = np.random.choice(
            spawn_points_m, num_CAV - num_CAV // 2, replace=False
        )
        ...
        spawn_point_m_h = np.random.choice(
            spawn_points_m, num_HDV - num_HDV // 2, replace=False
        )
        ...
        initial_speed = (
            np.random.rand(num_CAV + num_HDV) * 2 + 25
        )  # range from [25, 27]
        loc_noise = (
            np.random.rand(num_CAV + num_HDV) * 3 - 1.5
        )  # range from [-1.5, 1.5]
...
class MergeEnvContinuousMARL(MergeEnv):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update(
...
register(
    id="merge-multi-agent-continuous-v0",
    entry_point="highway_env.envs:MergeEnvContinuousMARL",
)
