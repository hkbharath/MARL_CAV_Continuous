...
from highway_env.vehicle.controller import MDPVehicle, MDPContinuousVehicle
...
class ContinuousAction(ActionType):
    ...
    ACCELERATION_RANGE = (-0.2, 0.2)
    """Acceleration range: [-x, x], in m/s²."""

    STEERING_RANGE = (-np.pi/225, np.pi/225) # approximately0.8 degrees
    ...
    @property
    def vehicle_class(self) -> Callable:
        return MDPContinuousVehicle if not self.dynamical else ControlledBicycleVehicle
